...
def obtain_attach_nodes_by_cluster_degree_all(args, edge_index, y_pred, cluster_centers, node_idxs, x, size, entropy):
    dis_weight = args.dis_weight
    entropy_weight = args.weight_entropy
    ...
    candidate_entropy = entropy[node_idxs].cpu().detach().numpy()

    candiadate_distances = max_norm(candiadate_distances)
    candiadate_degrees = max_norm(candiadate_degrees)
    candidate_entropy = max_norm(candidate_entropy)

    dis_score = candiadate_distances + dis_weight * candiadate_degrees - entropy_weight * candidate_entropy
...
def cluster_degree_selection(args, data, idx_train, idx_val, idx_clean_test, unlabeled_idx, train_edge_index, size, device,current_time):
    ...
    idx_attach = obtain_attach_nodes_by_cluster_degree_all(
        args, train_edge_index, y_pred, cluster_centers, unlabeled_idx.cpu().tolist(), encoder_x, size, entropy
    ).astype(int)
    #
    selected_nodes_foldpath = "./selected_nodes/{}/Overall/seed{}/{}".format(args.dataset, args.seed,current_time)
    if not os.path.exists(selected_nodes_foldpath):
        os.makedirs(selected_nodes_foldpath)

    selected_nodes_path = "./selected_nodes/{}/Overall/seed{}/{}/nodes.txt".format(args.dataset, args.seed,current_time)
