...
class GraphTrojanNet(nn.Module):
    ...
    def __init__(self, device, nfeat, nout, layernum=1, dropout=0.00):
        ...
        #     nn.Linear(nfeat, nout * nfeat),
        #     nn.ReLU(),
        #     nn.Linear(nout * nfeat, nout * nfeat),
        #     nn.Sigmoid()
        # )
...
class Backdoor:
    ...
    def fit(self, features, edge_index, edge_weight, labels, idx_train, idx_attach,idx_unlabeled):
        ...
        print("labels[idx_attach]",self.labels[idx_attach])
        # print(1/0)
        # num_classes = self.labels.max().item() + 1
        # num_selected = 1
        # selected_indices = idx_attach[torch.randperm(len(idx_attach))[:num_selected]]
        #
        # for idx in selected_indices:
        #     new_label = args.target_class
        #     while new_label == args.target_class:
        ...
        for i in range(args.trojan_epochs):
            ...
            for j in range(self.args.inner):
                ...
                # out_probabilities = F.softmax(output[idx_attach], dim=1)
                #
                # entropy_loss = 1 / torch.mean(-torch.sum(out_probabilities * torch.log(out_probabilities + 1e-10), dim=1) + 1e-10)
                # loss_inner += entropy_loss * self.args.weight_entropy
                loss_inner.backward()
            ...
            # out_probabilities = F.softmax(output[idx_outter], dim=1)
            # # loss_entropy = -entropy_loss(out_probabilities)
            # loss_entropy = 1 / torch.mean(-torch.sum(out_probabilities * torch.log(out_probabilities + 1e-10), dim=1) + 1e-10)

            idx_triggers = torch.arange(len(features), len(update_feat), device=features.device)

            idx_trojan = torch.cat([idx_outter, idx_triggers])
            out_probabilities = F.softmax(output[idx_trojan], dim=1)\

            target_probs = out_probabilities[:, args.target_class]
            non_target_probs = out_probabilities[:, torch.arange(out_probabilities.size(1)) != args.target_class]

            target_prob_loss = F.relu(0.5 - target_probs).mean()

            entropy_loss = 1 / torch.mean(-torch.sum(out_probabilities * torch.log(out_probabilities + 1e-10), dim=1) + 1e-10)
            alpha = 1
            beta = 100
            loss_entropy = alpha * target_prob_loss + beta * entropy_loss

            loss_outter = loss_target + self.args.homo_loss_weight * loss_homo

            # loss_outter += loss_entropy * self.args.weight_entropy
            ...
            if args.debug and i % 10 == 0:
                print('Epoch {}, loss_inner: {:.5f}, loss_target: {:.5f}, homo loss: {:.5f} ,entropy loss: {:.5f} '\
                        .format(i, loss_inner, loss_target, loss_homo, entropy_loss))
    ...
    def get_poisoned(self):
        ...
        poison_labels = self.labels.clone()


        # trigger_start_idx = len(self.labels)
        # trigger_end_idx = poison_x.shape[0]
        # num_trigger_nodes = poison_x.shape[0] - self.features.shape[0]
