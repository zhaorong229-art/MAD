...
def obtain_attach_nodes_by_cluster_degree_all(args, edge_index, y_pred,y, cluster_centers, node_idxs, x, size):
    dis_weight = args.dis_weight
    ...
    nontarget_nodes = np.where(y == args.target_class)[0]
...
def cluster_degree_selection(args, data, idx_train, idx_val, idx_clean_test, unlabeled_idx, train_edge_index, size, device,current_time):
    ...
    encoder_output = gcn_encoder(data.x, train_edge_index, None)
    probabilities = F.softmax(encoder_output, dim=1)
    entropy = -torch.sum(probabilities * torch.log(probabilities + 1e-10), dim=1)
    idx_attach = obtain_attach_nodes_by_cluster_degree_all(
        args, train_edge_index, y_pred,data.y, cluster_centers, unlabeled_idx.cpu().tolist(), encoder_x, size
    ).astype(int)
    #
...
def cluster_degree_selection_seperate_fixed(args, data, idx_train, idx_val, idx_clean_test, unlabeled_idx,
    ...
    y_pred = np.array(encoder_output.argmax(dim=1).cpu()).astype(int)
    cluster_centers = []
    each_class_size = int(size / (nclass - 1))  # Each class size for target class
    idx_attach = np.array([])

    # Now we will loop only over the target class (args.target_class)
    target_class = args.target_class  # Directly use the target class for idx_attach
    idx_sing_class = (y_pred == target_class).nonzero()[0]  # Select only target class nodes

    print("idx_sing_class", idx_sing_class)
    if len(idx_sing_class) == 0:
        return idx_attach

    print("current_class_size", size)

    # Save/load selected nodes for the target class
    ...
    if os.path.exists(selected_nodes_path):
        ...
        sing_idx_attach = sing_idx_attach[:size]
        idx_attach = np.concatenate((idx_attach, sing_idx_attach))
    else:
        kmedoids = KMeans(n_clusters=1, random_state=1)
        ...
        # Attach nodes based on cluster degree, specifically for the target class
        sing_idx_attach = obtain_attach_nodes_by_cluster_degree_single(
            args, train_edge_index, cluster_ids_x, sing_center, cand_idx_sing_class, encoder_x, size).astype(int)
