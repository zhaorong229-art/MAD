...
def prune_high_fluctuation_nodes(args, edge_index, edge_weights, x, device, large_graph=False):
    ...
    threshold = np.percentile(std.cpu().numpy(), 99.5)
    if args.fluctuation_thrd==0:
    # threshold = args.fluctuation_thrd
        print('threshold:',threshold)
        args.fluctuation_thrd = threshold

    high_std_nodes = torch.where(std >= args.fluctuation_thrd)[0]
...
def gnn_prune_low_entropy_nodes(args, poison_edge_index, poison_edge_weights, poison_x, poison_labels, device, large_graph=True):
    ...
    threshold = np.percentile(entropy.detach().cpu().numpy(), 0)
    if args.defense_thrd==0:
        # threshold = args.fluctuation_thrd
        print('threshold:',threshold)
        args.defense_thrd = threshold

    # mask = entropy < threshold
    low_entropy_target_class_mask = (entropy < args.defense_thrd) #& target_class_mask
    ...
    return filtered_poison_edge_index, filtered_poison_edge_weights, gnn_model, args.defense_thrd #,most_common_class#,poison_labels
