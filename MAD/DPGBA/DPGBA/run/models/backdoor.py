...
class Backdoor:
    ...
    def inject_trigger_arxiv(self, idx_attach, features, edge_index, edge_weight, device):
        ...
        trojan_feat, trojan_weights = self.trojan(features[idx_attach],
                                                  self.args.thrd)  # may revise the process of generate
        trojan_weights = torch.cat([trojan_weights,
                                    torch.ones([len(trojan_feat), 1 * self.args.trigger_size], dtype=torch.float,
    ...
    def fit(self, features, edge_index, edge_weight, labels, idx_train, idx_attach, idx_unlabeled):
        ...
        for i in range(args.trojan_epochs):
            for j in range(self.args.inner):
                ...
                trojan_feat, trojan_weights = self.trojan(features[idx_attach],
                                                          args.thrd)  # may revise the process of generate
                if args.dataset == 'ogbn-arxiv':
                    trojan_weights = torch.cat([trojan_weights,
                                                torch.ones([len(trojan_feat), 1 * self.args.trigger_size],
                                                           dtype=torch.float, device=self.device)], dim=1)
                else:
                    trojan_weights = torch.cat(
                        [torch.ones([len(trojan_feat), 1], dtype=torch.float, device=self.device), trojan_weights],
                        dim=1)
                trojan_weights = trojan_weights.flatten()
                trojan_feat = trojan_feat.view([-1, features.shape[1]])
                ...
                loss_inner = F.nll_loss(output[torch.cat([idx_train, idx_attach])],
                                        self.labels[torch.cat([idx_train, idx_attach])])  # add our adaptive loss
                loss_inner.backward()
                ...
                for k in range(self.args.k):
                    ...
                    ood_labels = torch.cat([torch.ones(len(trojan_feat), device=self.device),
                                            torch.zeros(len(trojan_feat), device=self.device)])
            ...
            if self.args.outter_size <= len(idx_unlabeled):
                idx_outter = torch.cat(
                    [idx_attach, idx_unlabeled[rs.choice(len(idx_unlabeled), size=4096, replace=False)]])
            else:
                idx_outter = torch.cat([idx_attach, idx_unlabeled])
            trojan_feat, trojan_weights = self.trojan(features[idx_outter],
                                                      self.args.thrd)  # may revise the process of generate
            if args.dataset == 'ogbn-arxiv':
                trojan_weights = torch.cat([trojan_weights, torch.ones([len(trojan_feat), 1 * self.args.trigger_size],
                                                                       dtype=torch.float, device=self.device)], dim=1)
            else:
                trojan_weights = torch.cat(
                    [torch.ones([len(idx_outter), 1], dtype=torch.float, device=self.device), trojan_weights], dim=1)
            trojan_weights = trojan_weights.flatten()
            trojan_feat = trojan_feat.view([-1, features.shape[1]])
            if args.dataset == 'ogbn-arxiv':
                trojan_edge = self.get_trojan_edge_arxiv(len(features), idx_outter, self.args.trigger_size).to(
                    self.device)
            else:
            ...
            losses = F.nll_loss(output[idx_outter], labels_outter[idx_outter],
                                reduction='none')  # This will be a tensor of the same shape as your batch
            loss_target = losses * (weights + 1)
            loss_target = loss_target.mean()
            sim = self.con_loss(all_features[idx_outter], all_features[idx_train], self.labels[idx_train],
                                output[idx_train])
            loss_targetclass = sim - self.simi(all_features[idx_outter], all_features[idx_outter]).fill_diagonal_(
                0).mean()
            loss_dis = F.nll_loss(output_detector[-len(trojan_feat):],
                                  torch.ones(len(trojan_feat), device=self.device).long())
            ...
            if args.debug and i % 50 == 0:
                print(
                    'Epoch {}, loss_inner: {:.5f}, loss_target: {:.5f},  loss_dis: {:.5f}, loss_diff: {:.5f}, loss_adv: {:.5f}, loss_targetclass: {:.5f}, ood_score: {:.5f} ' \
                    .format(i, loss_inner, loss_target, loss_dis, loss_dis, loss_target, loss_targetclass,
    ...
    def get_poisoned(self):
        with torch.no_grad():
            if self.args.dataset == 'ogbn-arxiv':
                poison_x, poison_edge_index, poison_edge_weights = self.inject_trigger_arxiv(self.idx_attach,
                                                                                             self.features,
                                                                                             self.edge_index,
                                                                                             self.edge_weights,
                                                                                             self.device)
                ...
                poison_x, poison_edge_index, poison_edge_weights = self.inject_trigger(self.idx_attach, self.features,
                                                                                       self.edge_index,
                                                                                       self.edge_weights, self.device)
        poison_labels = self.labels
        num_trigger_nodes = poison_x.shape[0] - self.features.shape[0]  # 新增触发器节点的数量
        # 将 poison_labels 的大小扩展以包含触发器节点
        poison_labels = torch.cat([poison_labels,
                                   torch.full((num_trigger_nodes,), self.args.target_class, dtype=torch.long,
                                              device=self.device)])
